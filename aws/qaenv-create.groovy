node {
    ...
    sh """
        /usr/local/bin/ansible-playbook --private-key ~/.ssh/camtest.pem "${home}/ansible/locust-ec2.yaml"
    """
    stage "install locust"
    sh """
        /usr/local/bin/ansible-playbook --private-key ~/.ssh/camtest.pem "${home}/ansible/locust-inst.yaml"
    """    
    }
    catch (err) {
        throw err
